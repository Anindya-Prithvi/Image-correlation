...
if len(tdirlist) == []:
    ...
    bf = cv2.BFMatcher()
    matchinatrix = [[None for i in range(len(dirlist))]
...
for i in range(np.size(matchinatrix, 0)):
    ...
    if max(lent) >= 10:
        print("The test image", tdirlist[i],
              "matches the most with", dirlist[varargma])
        showmostmatched = cv2.drawMatchesKnn(
            imagedbt[i],
            keypt[i],
            imagedb[varargma],
            keyp[varargma],
            matchinatrix[i][np.argmax(lent)],
            None,
            flags=2,
        )
